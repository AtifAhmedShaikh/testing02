...
import { deleteTodo, editTodo } from "../services/TodoService";
import { cloneTodo } from "../services/TodoService";
...
const Task: React.FC<TaskProps> = ({ task }) => {
  ...
  const handleCloneTask = async (username: string) => {
    await cloneTodo({ id: uuidv4(), username: taskToEdit });
    ...
    <tr key={task.id}>
      ...
      <td>
        ...
        <Modal modalOpen={openModalClone} setModalOpen={setOpenModalClone}>
          ...
          <div className="modal-action">
            <button
              onClick={() => handleCloneTask(taskToClone)}
              className="btn"
            >
              Yes
            </button>
